...
PROVIDERS: dict[str, dict] = {
    ...
    "openrouter": {
        ...
        "context_limit": 128000,
        "models": [],   # dynamic — any model on openrouter.ai
    },
    "custom": {
        "type":       "openai",
...
def messages_to_openai(messages: list, pass_images: bool = False) -> list:
    ...
    for m in messages:
        ...
        if role == "user":
            ...
            # for plain text turns (including empty-string history), keep the string.
            raw_content = m.get("content", "")
            content_val = (raw_content or None) if tcs else (raw_content or "")
            msg: dict = {"role": "assistant", "content": content_val}
            if tcs:
                msg["tool_calls"] = []
                for tc in tcs:
                    tc_msg = {
...
def stream_anthropic(
    ...
    # Wrap system prompt in a cached content block so Anthropic can cache the
    # full system + tool-list across turns (saves ~90% on those input tokens).
    ...
    kwargs = {
        ...
        "tools":      tool_schemas,  # cache_control on last entry set by registry.to_anthropic()
    }
...
def stream(
    ...
    """
    provider_name = config.get("_provider_override") or detect_provider(model)
    # For OpenRouter, pass the full org/model string (don't strip prefix)
    model_name    = model if provider_name == "openrouter" else bare_model(model)
